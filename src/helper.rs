...
/// Returns how many chunks `chunk_text` will split `text` into, given `lang`.
/// Useful to size a progress bar before synthesis starts.
pub fn count_chunks(text: &str, lang: &str) -> usize {
    let max_len = if lang == "ko" || lang == "ja" { 120 } else { 300 };
    chunk_text(text, Some(max_len)).len()
}
...
impl TextToSpeech {
    ...
    pub fn call(
        ...
        for (i, chunk) in chunks.iter().enumerate() {
            let reborrowed: Option<&mut dyn FnMut()> = match progress {
                Some(ref mut p) => Some(&mut **p),
                None => None,
            };
            let (wav, duration) = self._infer(&[chunk.clone()], &[lang.to_string()], style, total_step, speed, reborrowed)?;
    ...
    pub fn batch(
        ...
        self._infer(text_list, lang_list, style, total_step, speed, None)
    }
}
